...
/*
 * In order to generate the necessary key settings fields we need the admin to save the settings
 * now so that the stripe_currencies can be accessed now
 *
 * TODO: Will there be a dynamic way to do this using jquery?
 */
$stripe_currencies = $this->get_option('stripe_currencies');

/*
* Mod Multi-Currency START
*/

//TODO generate a full array of Stripe accepted currencies
$stripe_accepted_currencies = array('gbp','usd','eur');


$wc_stripe_settings_currencies = array(
	'stripe_currencies' => array(
		'title' => __('Currencies ', 'woocommerce'),
		'type' => 'multiselect',
		'class' => 'chosen_select',
		'css' => 'width: 350px;',
		'desc_tip' => __('Select the additional currencies you have Stripe API keys for.', 'woocommerce'),
		'options' => array(
			'usd' => 'US Dollar (USD)',
			'eur' => 'Euro (EUR)',
		...
		'default' => array('gbp'),
		//TODO Set default to whatever the store currency is
	),
);
/*
 * Generate the Stripe api key fields for each of the currencies
 */
if(is_array($stripe_currencies) && count($stripe_currencies) >=1 ) {

	$wc_stripe_settings_currency_keys = array();

	foreach($stripe_currencies AS $currency) {

		// Skip any currencies that managed to slip through but are not accepted by Stripe
		if(!in_array(strtolower($currency), $stripe_accepted_currencies)) {
			continue;
		}

		$label = strtoupper($currency).' Stripe Account: ';

		$wc_stripe_settings_currency_keys[] = array(
			($currency . '_test_publishable_key') => array(
				'title'       => __( $label.'Test Publishable Key', 'woocommerce-gateway-stripe' ),
				'type'        => 'password',
				'description' => __( 'Get your API keys from your stripe account.', 'woocommerce-gateway-stripe' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			($currency . '_test_secret_key')      => array(
				'title'       => __( $label.'Test Secret Key', 'woocommerce-gateway-stripe' ),
				'type'        => 'password',
				'description' => __( 'Get your API keys from your stripe account.', 'woocommerce-gateway-stripe' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			($currency . '_publishable_key')      => array(
				'title'       => __( $label.'Live Publishable Key', 'woocommerce-gateway-stripe' ),
				'type'        => 'password',
				'description' => __( 'Get your API keys from your stripe account.', 'woocommerce-gateway-stripe' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			($currency . '_secret_key')           => array(
				'title'       => __( $label.'Live Secret Key', 'woocommerce-gateway-stripe' ),
				'type'        => 'password',
				'description' => __( 'Get your API keys from your stripe account.', 'woocommerce-gateway-stripe' ),
				'default'     => '',
				'desc_tip'    => true,
...
$wc_stripe_settings_std_part1 = array(
	'enabled' => array(
...
$wc_stripe_settings_std_part2 = array(
...
$wc_stripe_settings_final = array_merge(
	$wc_stripe_settings_std_part1,
	$wc_stripe_settings_currencies);

foreach($wc_stripe_settings_currency_keys AS $key_set) {
	$wc_stripe_settings_final = array_merge($wc_stripe_settings_final, $key_set);
}

$wc_stripe_settings_final = array_merge(
	$wc_stripe_settings_final,
	$wc_stripe_settings_std_part2
);


return apply_filters( 'wc_stripe_settings',	$wc_stripe_settings_final);
